...
export function createUploadController() {
  ...
  async function postUpload(req, res, next) {
    ...
    try {
      ...
      const result = await uploadFile(fs.createReadStream(file.path), uploadName, isPrivate, uploadMime, { baseUrl: publicBaseUrl || proxyBaseUrl || baseUrl, sections, displayName, size: uploadStat.size });
      ...
      return res.status(201).json({ ok: true, data: { ...result, fileType: uploadMime, fileName: uploadName, size: uploadStat.size, convertedFrom: '' } });
    } catch (error) {
      return next(error);
    } finally {
