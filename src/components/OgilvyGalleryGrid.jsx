...
function getAspectClass(item, index, intrinsicAspectRatio) {
  const hint = String(item?.orientation || item?.ratio || '').toLowerCase();
  const ratio = intrinsicAspectRatio || getNumericRatio(item);
...
function GalleryItem({ item, index }) {
  ...
  const statusTone = item?.statusTone === 'video' ? 'border-sky-300/30 bg-sky-300/10 text-sky-200' : 'border-emerald-300/30 bg-emerald-300/10 text-emerald-200';
        ...
        >
          <div className="md:hidden">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {statusLabel ? (
                <span className={`rounded-full border px-2 py-0.5 text-[10px] tracking-[0.12em] ${statusTone}`}>
                  {statusLabel}
                </span>
              ) : null}
            </div>
...
function OgilvyGalleryGrid({ items = [] }) {
  ...
  return <div className="columns-2 gap-1 md:columns-3 lg:columns-4">{items.map((item, index) => <GalleryItem key={item.id} item={item} index={index} />)}</div>;
}
