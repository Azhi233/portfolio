    ...
    <main className="min-h-screen bg-[#f7f6f1] text-[#161616]">
      ...
      <section className="flex min-h-screen items-center justify-center px-6 pt-10 md:pt-0">
        <div className="flex w-full max-w-[210px] flex-col items-center gap-3">
          <AccessIcon />
          <form onSubmit={(event) => { event.preventDefault(); unlock(password); }} className="w-full">
            <div className="flex h-[40px] items-center border border-[#e6e0d8] bg-white px-3 shadow-[0_1px_2px_rgba(0,0,0,0.03)]">
              <Input
                type="password"
                value={password}
                onChange={(event) => { setPassword(event.target.value); setError(''); setHint(''); }}
              ...
              />
              <Button type="submit" variant="subtle" className="ml-1.5 h-auto border-0 bg-transparent p-0 text-[24px] leading-none text-[#ded8cf] hover:bg-transparent hover:text-[#cdc6bb]">→</Button>
            </div>
          </form>
