  ...
  return (
    <>
      ...
      <Modal open={state.isOpen} title="Homepage Video" onClose={() => setState((prev) => ({ ...prev, isOpen: false }))}>
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1.15fr)_minmax(380px,0.85fr)] xl:grid-cols-[minmax(0,1.2fr)_minmax(420px,0.8fr)] 2xl:grid-cols-[minmax(0,1.2fr)_minmax(440px,0.8fr)]">
          <div className="grid gap-4">
            <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-4 sm:p-5 lg:p-6">
              <div className="mb-3 flex items-center justify-between gap-3">
                <div>
                  <p className="text-[11px] uppercase tracking-[0.18em] text-white/55">Step 1</p>
                  <h3 className="mt-1 text-sm tracking-[0.08em] text-white">Upload</h3>
                </div>
                <span className="rounded-full border border-cyan-400/20 bg-cyan-400/10 px-3 py-1 text-[11px] tracking-[0.14em] text-cyan-200">VIDEO ONLY</span>
              </div>
              <MediaPicker label="Upload Homepage Video" accept="video/*" onPick={uploadHomeVideo} value={state.draft?.homeVideoUrl} uploading={state.uploading} helperText="Select a homepage hero video to begin the upload flow." />
            </div>

            <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-4 sm:p-5 lg:p-6">
              <div className="mb-3 flex items-center justify-between gap-3">
                <div>
                  <p className="text-[11px] uppercase tracking-[0.18em] text-white/55">Current Value</p>
                  <h3 className="mt-1 text-sm tracking-[0.08em] text-white">Homepage video content</h3>
                </div>
              </div>
              <div className="grid gap-3 text-sm text-white/75 sm:grid-cols-2">
                <div className="rounded-xl border border-white/10 bg-black/20 px-3 py-3">
                  <p className="text-[11px] uppercase tracking-[0.14em] text-white/45">Caption</p>
                  <p className="mt-1 break-all text-white">{state.draft?.homeVideoTitle || 'No homepage video selected yet.'}</p>
                </div>
                <div className="rounded-xl border border-white/10 bg-black/20 px-3 py-3">
                  <p className="text-[11px] uppercase tracking-[0.14em] text-white/45">URL</p>
                  <p className="mt-1 break-all text-white/70">{state.draft?.homeVideoUrl || 'Upload a looping hero video for the homepage.'}</p>
                </div>
              </div>
            </div>
          </div>

          <div className="rounded-3xl border border-white/10 bg-black/25 p-4 sm:p-5 lg:p-6 lg:pr-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-[11px] uppercase tracking-[0.18em] text-white/55">Progress</p>
                <p className="mt-1 text-sm tracking-[0.08em] text-white">{stageLabel}</p>
              </div>
              <div className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-[11px] tracking-[0.14em] text-white/60">
            ...
            <div className="mt-4 grid gap-3">
              {steps.map((step) => {
                ...
                return (
                  <div key={step.id} className="flex items-start justify-between gap-3 rounded-xl border border-white/10 bg-black/20 px-3 py-3">
                    <div className="flex items-start gap-3">
                      <StepDot state={stepState} />
                      <div>
                        <p className="text-sm tracking-[0.08em] text-white">{step.label}</p>
                        <p className="mt-1 text-xs leading-5 text-white/60">{step.id === 'select' ? '选择本地视频文件。' : step.id === 'transcode' ? '转码成可播放的 MP4。' : step.id === 'upload' ? '上传到存储。' : step.id === 'writeback' ? '写回首页配置。' : '流程结束。'}</p>
                      </div>
                    </div>
            ...
            <div className="mt-4 rounded-xl border border-white/10 bg-black/30 px-3 py-3 text-xs leading-5 text-white/75">
